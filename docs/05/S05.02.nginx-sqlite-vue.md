...
- 2: puis initializer la bdd avec des data
- 3: Voir les données sur la page
...
et dans ce repertoire créez les repertoire `data` et `html`
...
La base de donnée sera vide mais on verra comment y accéder.
...
Ce hack crée un processus de fond qui toune indéfiniment.

On pourrait aussi utiliser comme commande `command: tail -f /dev/null`.
