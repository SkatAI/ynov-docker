...
puis construisez l'image `ynov:hello-world` depuis ce Dockerfile
...
Le fichier Dockerfile de solution est sur le github : <https://raw.githubusercontent.com/SkatAI/ynov-docker/refs/heads/master/dockerfiles/ubuntu-nginx-server.docker>
...
- vérifiez que le process nginx tourne bien (ps aux | grep nginx)

Enfin allez sur `localhost:8080` et vérifiez que vous voyez bien votre page web.

--------

!!! le container s'arrête tout de suite meme en mode -d
