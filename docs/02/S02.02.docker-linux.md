...
1. **Téléchargez l'image Debian**
   - Utilisez la commande Docker appropriée pour télécharger la dernière image Debian.
...
6. **Attachez-vous au conteneur en cours d'exécution**
   ...
   - Exécutez quelques commandes Linux simples comme `date`, `whoami` et `hostname`.
   - Quittez le conteneur sans l'arrêter (Astuce : Utilisez la séquence d'échappement).
